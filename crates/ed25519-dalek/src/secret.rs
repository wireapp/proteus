...
impl zeroize::Zeroize for ExpandedSecretKey {
    fn zeroize(&mut self) {
        self.key.zeroize();
        self.nonce.zeroize()
    }
...
impl ExpandedSecretKey {
    ...
    #[inline]
    /// Don't ever do this
    pub unsafe fn from_bytes_unchecked(bytes: &[u8]) -> Result<ExpandedSecretKey, SignatureError> {
        if bytes.len() != EXPANDED_SECRET_KEY_LENGTH {
            return Err(InternalError::BytesLengthError {
                name: "ExpandedSecretKey",
                length: EXPANDED_SECRET_KEY_LENGTH,
        ...
        Ok(ExpandedSecretKey {
            key: Scalar::from_bits_unchecked(lower),
            nonce: upper,
        })
    }
        ...
        D: Digest<OutputSize = U64>,
    {
        ...
        use curve25519_dalek::digest::Update as _;
